...
class ProjectTaskNativeSchedulerCalendar(models.Model):
    ...
    def make_res_cal_leave(self, task_resource_ids, t_params, task_ids):
        ...
        cal_id = str(project.resource_calendar_id.id) if project.resource_calendar_id else ""
        task_res = []
        ...
        if task_resource_ids:
            attendance_ids, leave_ids, task_res = self.get_leave_for_resource(
                task_res, task_resource_ids, attendance_ids, leave_ids, project, task_ids
            )
